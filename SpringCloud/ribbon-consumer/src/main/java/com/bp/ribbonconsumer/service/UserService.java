...
public class UserService {
    ...
    public User getUserBySyncGET(Integer id){
        ...
        User user = restTemplate.getForObject("http://hello-service/get-user/{1}", User.class, id);
        return user;
    }
    ...
    @HystrixCommand(fallbackMethod = "userBack",
            groupKey = "user",
            commandKey = "getUserByAsyncGET",
            commandProperties = {
                    @HystrixProperty(name = "execution.isolation.thread.timeoutInMilliseconds", value = "20000")
            })
    public Future<User> getUserByAsyncGET(final Integer id){
        //异步的方式请求数据
        return new AsyncResult<User>() {
            @Override
            public User invoke() {
                User user = restTemplate.getForObject("http://hello-service/post-user?id={1}", User.class, id);
